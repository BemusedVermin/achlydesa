...
fn main() {
    let mut sim = Simulation::new(Setup { width: 64, height: 48, seed: 2026, warmup: 300, npcs: 80, ..Default::default() });
    let topo = sim.substrate().topology().clone();

    {
        let cat = sim.feature_catalog();
        let feats = sim.features();
        println!("World 64×48, seed 2026 — {} features placed\n", feats.total());
        ...
        for i in topo.indices() {
            ...
            let cats: std::collections::HashSet<Category> = here.iter().map(|f| cat.def(f.kind).category).collect();
            if cats.len() >= 2 {
                stacked.push((i, here.iter().map(|f| cat.name(f.kind)).collect()));
            }
        }
        println!("\n{} hexes stack two or more categories. A few:", stacked.len());
    ...
    sim.run(400);
    let skill_names: Vec<String> = (0..sim.registry().skill_count()).map(|s| sim.registry().skill(s).name.clone()).collect();
    let c = sim.census();
    println!("\n--- after 400 days ({} NPCs born) ---", 80);
    println!("population {}, coins {}, goods {}", c.population, c.money, c.goods);
    println!("emergent professions (born to a calling, deepened by doing):");
    for (id, &n) in c.professions.iter().enumerate() {
        println!("    {:<10} {n} practising", skill_names.get(id).map(String::as_str).unwrap_or("?"));
    }
    println!(
