...
pub fn plan_for(name: &str, category: Category) -> Plan {
    ...
    match name {
        ...
        "monastery" | "temple" | "high_temple" | "temple_of_the_demiurge" => return Plan::Landmark(Temple),
        ...
        "pillar_of_the_boast" | "effaced_monument" | "salt_pillars" | "gate_of_the_seven" | "toppled_arch" | "archon_throne" => {
            return Plan::Landmark(Obelisk);
        }
        "barrow" | "bone_orchard" | "serpent_mound" | "plague_pit" => return Plan::Landmark(Cairn),
        "frontier_fort" | "barons_keep" | "warlords_redoubt" | "ruined_keep" => return Plan::Landmark(Keep),
        _ => {}
    }
    match category {
...
fn settlement_tier(name: &str) -> u8 {
    if name.contains("city") {
        ...
        3
    } else if name.contains("village") || name.contains("market") || name.contains("waystation") || name.contains("roost") || name.contains("colony") {
        2
    } else {
        1
    }
...
fn court_prop(name: &str) -> Prop {
    if name.contains("temple") || name.contains("shrine") || name.contains("almshouse") || name.contains("mendicant") || name.contains("hall") {
        Prop::Temple
    ...
    for c in tiles {
        ...
        for f in feats {
            ...
            match plan_for(&def.name, def.category) {
                Plan::Settlement(tier) => place_settlement(commands, lib, centre, top, tier, &mut rng),
                Plan::Landmark(prop) => {
                    ...
                    let off = Vec2::new(ang.cos(), ang.sin()) * HEX_R * 0.16 * (f.kind.min(3) as f32);
                    place_one(commands, lib, prop, centre + off, top, &mut rng, (0.95, 1.15));
                }
                Plan::Nothing => {}
            }
        }
...
fn place_settlement(commands: &mut Commands, lib: &PropLibrary, centre: Vec2, top: f32, tier: u8, rng: &mut Rng) {
    ...
    for _ in 0..count {
        ...
        let prop = if rng.chance(0.45) { Prop::Hut } else { Prop::House };
...
fn place_one(commands: &mut Commands, lib: &PropLibrary, prop: Prop, pos: Vec2, top: f32, rng: &mut Rng, scale: (f32, f32)) {
...
mod tests {
    ...
    fn registry_maps_signature_and_fallback() {
        assert!(matches!(plan_for("city", Category::Community), Plan::Settlement(4)));
        assert!(matches!(plan_for("thorp", Category::Community), Plan::Settlement(1)));
        assert!(matches!(plan_for("pale_beacon", Category::Wilderness), Plan::Landmark(Prop::Beacon)));
        assert!(matches!(plan_for("standing_stones", Category::Wilderness), Plan::Landmark(Prop::StoneRing)));
        assert!(matches!(plan_for("temple_of_the_demiurge", Category::Court), Plan::Landmark(Prop::Temple)));
        // Unknown kinds still resolve through the category fallback.
        assert!(matches!(plan_for("some_new_wonder", Category::Wilderness), Plan::Nothing));
        assert!(matches!(plan_for("some_new_ruin", Category::Ruin), Plan::Landmark(Prop::Ruin)));
        assert!(matches!(plan_for("some_new_court", Category::Court), Plan::Landmark(_)));
    }
}
